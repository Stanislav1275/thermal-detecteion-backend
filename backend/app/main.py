...
@app.get("/")
async def root():
    return {"message": "Thermal Person Detection API", "version": API_VERSION, "status": "running"}
...
def validate_confidence(confidence: float) -> None:
    if not isinstance(confidence, (int, float)):
        raise HTTPException(status_code=400, detail="confidence_threshold должен быть числом")

    if not 0.0 <= confidence <= 1.0:
        raise HTTPException(
            status_code=400, detail="confidence_threshold должен быть в диапазоне от 0.0 до 1.0"
        )
...
async def upload_images(
    ...
    if detector is None or processor is None:
        raise HTTPException(status_code=503, detail="Детектор не инициализирован")

    # Type narrowing: processor is guaranteed to be not None after the check above
    ...
    try:
                ...
                try:
                    ...
                    if not extracted:
                        extraction_errors.append(
                            f"Архив {sanitized_filename} не содержит валидных изображений"
                        )
                        continue
                    for _archive_path, extracted_path in extracted:
                        all_image_files.append(extracted_path)
                except Exception as e:
                    extraction_errors.append(
                        f"Ошибка при распаковке архива {sanitized_filename}: {str(e)}"
                    )
                    continue
        ...
        if not all_image_files:
            error_msg = (
                "Нет валидных изображений. Поддерживаемые форматы: TIFF, PNG, JPEG, WEBP, ZIP"
            )
        ...
        job_id = storage.create_job(
            name=name, confidence_threshold=confidence_value, total_images=len(all_image_files)
        )
...
async def process_images_from_paths_task(
    job_id: str, image_paths: List[str], confidence_threshold: float, temp_dir: str
):
    global detector, processor
    ...
    try:
        ...
        processed_count = 0
        detections_count = 0
        used_filenames: dict[str, int] = {}  # Словарь для отслеживания использованных имен файлов
        ...
        for image_path in image_paths:
            try:
                ...
                result = processor.process_image(
                    str(input_path), str(output_path), confidence=confidence_threshold
                )
                ...
                error_result = ImageResult(
                    filename=filename, detections=[], success=False, error=str(e)
                )
...
async def delete_job(job_id: str):
    ...
    return DeleteJobResponse(message="Задача успешно удалена", job_id=job_id)
...
async def download_results_zip(
    ...
    if status["status"] != "completed":
        raise HTTPException(
            status_code=400, detail=f"Задача еще не завершена. Текущий статус: {status['status']}"
        )

    if not 0.0 <= min_confidence <= 1.0:
        raise HTTPException(
            status_code=400, detail="min_confidence должен быть в диапазоне от 0.0 до 1.0"
        )
    ...
    return FileResponse(str(zip_path), media_type="application/zip", filename=zip_filename)
