...
class ImageProcessor:
    ...
    def process_image(
        self, image_path: str, output_path: str, confidence: Optional[float] = None
    ) -> ImageResult:
        try:
            ...
            if test_img is None:
                raise ValueError(
                    f"Не удалось прочитать изображение: {image_path}. Файл может быть поврежден или иметь неподдерживаемый формат."
                )
            ...
            return ImageResult(
                filename=os.path.basename(image_path), detections=detections, success=True
            )
            ...
            if "need at least one array to stack" in error_msg.lower():
                error_msg = (
                    f"Изображение некорректно или повреждено: {os.path.basename(image_path)}"
                )
            return ImageResult(
                filename=os.path.basename(image_path), detections=[], success=False, error=error_msg
            )
    ...
    def extract_zip_archive(zip_data: bytes, extract_to: str) -> List[Tuple[str, str]]:
        ...
        try:
                    ...
                    try:
                        ...
                        if test_img is None:
                            ...
                            extraction_errors.append(
                                f"{member}: не удалось прочитать как изображение"
                            )
                            continue
            ...
            raise ValueError(f"Ошибка при распаковке архива: {e}") from e
